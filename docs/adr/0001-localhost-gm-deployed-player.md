...
**Consequences:** A player who runs `quasar dev` locally gets GM powers — acceptable by convention. All campaign tools must respect the same mode check. GM-only surfaces (prep lessons, heart editing) gate on hostname, not on a user identity.
