...
- Universal weak save = **Reflex** (all expert Fort/Will, only trained Reflex). Book 1 insect/area attacks lean on basic Reflex.
- Swarms have **weakness to area/splash**, resist physical, immune to precision → only Yggostrus and Cy'An have real anti-swarm AoE; both eidolons + melee crew are resisted.
- **Two summoners** = two separate shared-HP failure points (Anya AC 14; Yggostrus Tiny AC 15). Map also gets crowded (beast + dragon + construct).
- **Soralyon** = guardians/monuments/magic (NOT masks) — maps onto guarding the Magaambya and the rune-covered oread Stone Ghost. Taraan is the natural moral center of the Ch 3 finale.
- Party is healing-rich (Font + Healing Plaster + Fruit Leshy fruit).
- Non-lethal Perquisite mandate clashes with the party's lethal AoE — intentional Uduak mirror.

**Gap:** Magaambya branch picks are not stored in PathBuilder; confirm at session zero. Anya's PathBuilder export is incomplete (spells, one Natural Skill pick).

Captured in lesson `lessons/0008-party-character-analysis-gm-prep.html`.
